...
给你n个工人和m个任务，还有一个利润p，每个任务需要用到x\[i\]个工人，利润为y\[i\]，问你分配工人后，有多少种方案数可以**让利润不小于p**，答案取模1e9+7；  
T组    
****1&lt;=T&lt;=100  
1&lt;=n&lt;=100  
1&lt;=m&lt;=100  
...
1&lt;=y\[i\]&lt;=100  
1 &lt;= p &lt;= 100  
 第一行输入T 接下来每组输入m n p 然后输入x\[i\] 接下来一行输入y\[i\] 样例 输入：  
 1  
 2 5 3  
 2 2  
 2 3  
...
**发现这道题不同于普通的背包问题，不是求最大利润，而是求不小于某个利润的方案数**

**dp\[j\]\[k\]为利润为j且工人为k的方案数**
...
dp[0][0] = 1;
for(int i = 0; i < m; i++){
    for(int j = S; j >= y[i]; j--){ //注意为什么要倒着算方案数
        for(int k = n; k >= x[i]; k--){

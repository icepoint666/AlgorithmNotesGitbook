...
类似于之前的**动态规划编辑距离**，这道题更简单，只用判断一次编辑内是否能相等即可

还是类似的思路：四种情况，假设只在s1上编辑

* ①如果当前s1\[i\] == s2\[j\]，那么i++, j++
* ②如果当前s1\[i\] != s2\[j\]，会消耗一次次数，如果次数不够了直接false, 如果够那么判断后面的
  * 如果s1\[i\] == s2\[j+1\]，那么只用s1插入一个s2\[j\]字符，j++
  * 如果s1\[i+1\] == s2\[j\]，那么只用s1删除一个s1\[i\]字符,   i++
  * 如果都不是，那么就修改， i++, j++
* 如果到两个字符串至少有一个结束了
  * 如果没有同时到达末尾而且没有剩余操作次数，那么false
  * 如果没有同时到达末尾还有剩余操作次数，如果只差一个长度的话，表示还可以插入or删除,true
  * 如果同时到达末尾，那么true

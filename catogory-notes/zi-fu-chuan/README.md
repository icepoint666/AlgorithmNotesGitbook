...
| 面试题01.03 | URL化 | 原地修改字符串 | 记忆，小坑 |
| 面试题01.09 | 字符串轮转 | find查找子串的原理+应用 | 记忆 |
...
**题意：** 给定两个字符串`s1`和`s2`，请编写代码检查`s2`是否为`s1`旋转而成（比如，`waterbottle`是`erbottlewat`旋转后的字符串

**方法1：**直接利用string find\(\)方法查找子串，可以用一行解决问题
...
先将字符串2复制\(s2+s2\)，但是可以不用实现这个操作，在标记下标的时候%n就好了

两个字符串从开始匹配，找到相同的字符，然后两个字符就一个一个字符的匹配
...
"waterwateb"
"watebwater"
会发生匹配到r!=b匹配失败的情况，这时候i需要退回到直到能与b匹配的地方，如果找不到与b匹配的地方
i就会退回到0也就是w，然后与j的下一个开始继续匹配
```
...
bool isFlipedString(string s1, string s2) {
    ...
    int n = s1.size();
    if(n == 0)return true;
    int i = 0, j = 0, cnt = 0, last_i = 0;
    while(i < n && j < 2 * n){
        if(s1[i] != s2[j%n]){ //相当于把字符串接在了后面，只用%n
            while(i > 0 && s1[i--]!=s2[j%n]); //匹配失败i往前回溯，找到相等的字符
            j++;
            cnt = 0;
        }else{
            i++;
